...
image_octave_s *dog_octaves = NULL, *mag_pyr = NULL, *grad_pyr = NULL;
...
mat_s *image_scale_init(mat_s *src)
{
	...
	else
	{
		...
		mat_free(img_blur);
		__image_show_f32((char *)"dst", dst, 1);
	}
	return dst;
}
...
{
	image_octave_s *octaves = NULL;
	mat_s *dst = NULL, *temp = NULL;
	mat_s *tmp_test = NULL;
	mat_s *img_tmp = mat_create(src->rows, src->cols, MAT_F32);
	int i, j;
	float k;
	...
	mat_clone_f32(src,img_tmp);

#if 0
	tmp_test = src;
	image_octave_s octaves_test;
	printf("src_addr = 0x%x  src_buf addr= 0x%x \n", src,src->buf);
	printf("tmp_test_addr = 0x%x  tmp_test_buf addr= 0x%x \n", tmp_test, tmp_test->buf);
//	write_mat_data_to_file_f32("src.dat",src);
//	write_mat_data_to_file_f32("level.dat", octaves_test.octave.level);
	octaves_test.octave.level = src;
	__image_show_f32("src", src, 1);
	printf("octaves_test.octave.level_addr  = 0x%x octaves_test.octave.level_buf_addr  = 0x%x\n", octaves_test.octave.level, octaves_test.octave.level->buf);
	__image_show_f32("octaves_test.octave->level", octaves_test.octave.level, 1);
#endif
	...
	for (i = 0; i < num_octaves; i++)
	{
		...
		__image_show_f32((char *)"dog", temp, 1);
		img_tmp = mat_resize_half_f32(((octaves[i].octave)[SCALESPEROCTAVE].level));
		__image_show_f32((char *)"level", img_tmp, 1);
	}
	mat_free(img_tmp);
	return octaves;
}
...
bool select_point(image_octave_s *gaussian_pyr)
{
	return true;
}
